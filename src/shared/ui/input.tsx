...
const CustomInput: FC<Props> = ( { type, name, label, error, className, ...others } ) => {
  return (
    <Field className={twMerge( 'relative', className )}>
      ...
      <Input type={type} name={name} className={twMerge( 'block w-full px-3 py-2 outline-none border rounded-md bg-transparent placeholder:text-zinc-500 focus:border-zinc-500', error.name === name ? 'border-red-400' : 'border-zinc-300' )} required {...others} />
